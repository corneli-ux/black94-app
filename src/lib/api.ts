...
/**
 * Fetch active ad campaigns for display in profile screens and feed.
 * Returns up to `limit` campaigns ordered by creation date descending.
 */
export async function fetchActiveAdCampaigns(limit = 5): Promise<AdCampaign[]> {
  try {
    const snap = await firestore()
    ...
    return snap.docs.map((doc: any) => {
      ...
      return {
        ...
        advertiserId: typeof d.advertiserId === 'string' ? d.advertiserId : '',
        status: typeof d.status === 'string' ? d.status : 'active',
        createdAt: (() => { try { return tsToMillis(d.createdAt); } catch { return Date.now(); } })(),
      };
    });
  } catch (e) {
    if (__DEV__) console.warn('[Ads] Failed to fetch active campaigns:', e);
    return [];
  }
}
