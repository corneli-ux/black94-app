...
function MediaGrid({ posts, navigation }: { posts: Post[]; navigation: any }) {
  ...
  if (postsWithMedia.length === 0) {
    return (
      <View style={{ alignItems: 'center', paddingTop: 60 }}>
        <AppIcon name="photo-library" size="hero" color={colors.textSecondary} style={{ marginBottom: 12 }} />
        ...
        >
          ...
          {post.mediaUrls.length > 1 && (
            <View style={{ position: 'absolute', top: 6, right: 6, backgroundColor: colors.overlayDark, borderRadius: 4, paddingHorizontal: 5, paddingVertical: 2 }}>
              <AppIcon name="photo-library" size="xs" color={colors.white} />
            </View>
          )}
        </TouchableOpacity>
      ))}
...
const styles = StyleSheet.create({
  ...
  editProfileBtnText: { color: colors.accent, fontWeight: '700', fontSize: 13.5, letterSpacing: 0.2 },
  followBtn: {
  ...
  followBtnText: { color: colors.bg, fontWeight: '800', fontSize: 13.5, letterSpacing: 0.2 },
  ...
  messageBtnText: { color: colors.white, fontWeight: '700', fontSize: 13.5 },
