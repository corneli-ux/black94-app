          ...
          {(post.mediaUrls?.length > 0) && (
            <TouchableOpacity activeOpacity={0.95} onPress={handleDoubleTap}>
              <View style={profileCardStyles.mediaContainer}>
                <Image source={{ uri: post.mediaUrls[0] }} style={profileCardStyles.media} resizeMode="cover" onLoad={() => setHasMediaError(false)} onError={(e) => { console.warn('[ProfileSelf] Image failed:', post.mediaUrls[0]?.slice(0, 80), e.nativeEvent?.error); setHasMediaError(true); }} />
                {hasMediaError && (
